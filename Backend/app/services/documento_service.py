...
def listar_documentos(db: Session) -> list[Documento]:
    return db.query(Documento).filter(Documento.tipo == "publico").order_by(Documento.creado_en.desc()).all()


def listar_documentos_publico(db: Session, limite: int = 6) -> list[Documento]:
    return (
        db.query(Documento)
        .filter(Documento.estado == EstadoDocumentoEnum.indexado, Documento.tipo == "publico")
