...
from app.schemas.common import MensajeOk, R_401, R_403, R_404
...
class TicketSchema(BaseModel):
    ...
    @classmethod
    def from_model(cls, t: TicketVacio, usuario: Optional[Usuario] = None) -> "TicketSchema":
        return cls(
            id=str(t.id),
            pregunta=t.pregunta,
...
class MensajeSchema(BaseModel):
    ...
    @classmethod
    def from_model(cls, m) -> "MensajeSchema":
        return cls(
            id=str(m.id),
...
class ConteoNoLeidos(BaseModel):
    count: int
...
@router.get(
    "/usuario/no-leidos",
    response_model=ConteoNoLeidos,
    summary="Contar mensajes no leídos del usuario",
    responses={**R_401},
)
def no_leidos(
...
def mis_tickets(
    ...
    usuario_id = current_user.get("_usuario_id", "")
    tickets = (
        db.query(TicketVacio)
        .filter(TicketVacio.usuario_id == usuario_id)
...
@router.get(
    "",
    response_model=list[TicketSchema],
    summary="Listar todos los tickets (admin)",
    description="Filtrá por estado: `pendiente`, `revisado` o `respondido`.",
...
@router.put(
    ...
    summary="Marcar ticket como revisado",
...
@router.patch(
    ...
    status_code=200,
    summary="Marcar mensajes del ticket como leídos",
    responses={**R_401},
)
def marcar_ticket_leido(
    ticket_id: str,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
...
@router.get(
    ...
    description="Retorna los mensajes y los marca como leídos para el rol del usuario actual.",
    responses={**R_401},
)
def listar_mensajes(
    ticket_id: str,
...
@router.post(
    ...
    status_code=201,
    summary="Enviar mensaje en un ticket",
    responses={
